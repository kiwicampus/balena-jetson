...
PINMUXCFG = "tegra234-mb1-bct-pinmux-p3701-0000-a04.dtsi"
LNXSIZE ?= "67108864"
# DTBNAME = "tegra194-p2888-0001-p2822-0000"
DTBNAME_kiwi-orin = "tegra234-p3701-0000-p3737-0000"
...
signfile() {
    ...
    export bins="mb2_bootloader nvtboot_cpurf_t234.bin; \
        ...
        bpmp_fw_dtb tegra234-bpmp-3701-0000-3737-0000.dtb; \
        spe_fw spe_t234.bin; \
    ...
    --cmd "sign$1" \
    --cfg flash.xml.in \
    --uphy_config tegra234-mb1-bct-uphy-lanes-p3701-0000.dts \
    --device_config tegra234-mb1-bct-device-p3701-0000.dts \
    --misc_config tegra234-mb1-bct-misc-p3701-0000.dts \
    --misc_cold_boot_config tegra234-mb1-bct-misc-p3701-0000.dts \
    ...
    --scr_cold_boot_config tegra234-mb2-bct-scr-p3701-0000.dts \
    --br_cmd_config tegra234-mb1-bct-reset-p3701-0000.dts \
    --dev_params tegra234-br-bct-p3701-0000.dts --bins "${bins}"
}
...
do_configure() {
    ...
    ln -s "${DEPLOY_DIR_IMAGE}/tos-${MACHINE}.img" ./tos-mon-only_t234.img
    ...
    for f in ${STAGING_DATADIR}/tegraflash/tegra23[4x]-*.cfg; do
        ln -s $f .
    ...
    sed -i -e "s/\[DTB_NAME\]/$(echo ${DTBFILE} | cut -d '.' -f 1)/g" ${WORKDIR}/partition_specification234.txt
    ...
    cp ${WORKDIR}/resinOS-flash234.xml ${DEPLOY_DIR_IMAGE}/bootfiles/flash.xml
    ...
    # mb1 (a + b)
    dd if=${DEPLOY_DIR_IMAGE}/bootfiles/mb1_t234_prod_sigheader.bin.encrypt of=boot0.img seek=32768 bs=1 conv=notrunc
    dd if=${DEPLOY_DIR_IMAGE}/bootfiles/mb1_t234_prod_sigheader.bin.encrypt of=boot0.img seek=294912 bs=1 conv=notrunc #(32768 + 262144 which is mb1 size)
    ...
    # spe-fw (a + b)
    dd if=${DEPLOY_DIR_IMAGE}/bootfiles/spe_t234_sigheader.bin.encrypt of=boot0.img seek=1130496 bs=1 conv=notrunc
    dd if=${DEPLOY_DIR_IMAGE}/bootfiles/spe_t234_sigheader.bin.encrypt of=boot0.img seek=1261568 bs=1 conv=notrunc

    # mb2 (a + b)
    dd if=${DEPLOY_DIR_IMAGE}/bootfiles/nvtboot_cpurf_t234_sigheader.bin.encrypt of=boot0.img seek=1392640 bs=1 conv=notrunc
    dd if=${DEPLOY_DIR_IMAGE}/bootfiles/nvtboot_cpurf_t234_sigheader.bin.encrypt of=boot0.img seek=1654784 bs=1 conv=notrunc
...
do_install() {
    ...
    cp ${WORKDIR}/partition_specification234.txt ${D}/${BINARY_INSTALL_PATH}/
